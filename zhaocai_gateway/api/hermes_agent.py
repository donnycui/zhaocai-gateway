...
from zhaocai_gateway.services import HermesConfigCompilerService, HermesPairingService
...
def create_hermes_agent_router(store: SQLiteStore) -> APIRouter:
    pairing_service = HermesPairingService(store)
    compiler_service = HermesConfigCompilerService(store)
    router = APIRouter(prefix="/hermes-agent/v1", tags=["hermes-agent"])
    ...
    def get_config_meta(authorization: str | None = Header(default=None)) -> dict:
        ...
        device = store.get_hermes_device_by_sync_token_hash(_hash_token(raw_token))
        if device is None:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Invalid sync token",
    ...
    def post_config_applied(
        ...
        raw_token = _extract_bearer_token(authorization)
        report = store.record_applied_hermes_config(
